        ...
        {props.isCorrect && props.message.content.includes("That's correct") ? 
            <div>
                ...
                <div ref={props.ref} className={`m-0 flex ${props.message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`rounded-[12px] px-6 py-4 m-0 max-w-[70%] ${props.message.role === 'user' ? 'bg-[#D5E6ED] text-[#084762]' : 'bg-gray-200'}`}>
                        {props.message.content}
                    </div>
                </div>
            </div> :
            <div ref={props.ref} className={`m-0 flex ${props.message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`rounded-[12px] px-6 py-3 m-0 max-w-[70%] ${props.message.role === 'user' ? 'bg-[#D5E6ED] text-[#084762]' : 'bg-gray-200'}`}>
                    {props.message.content}
                </div>
            </div>
        }
