...
export default function Home() {  
  ...
  // tracks the specific problem the student is on 
  ...
  const handleClick = (num) => {
    ...
    console.log("questions[num] = " + questions[num])
    ...
    setSystemPrompt(prev => ({
        ...
        + `You can give hints when the user responds with "I don't know" or a similar response. Only allow yourself to give one hint. Your replies are under 500 characters. Make sure to only say the student’s answer is correct if they get it almost right.`
        + `Only consider a student's answer as correct if they are able to send you the definition/answer. Do not compile the correct answer from previous user responses.` 
        + `Once the student’s answer is deemed correct you can stop replying until further prompting. Here is the question that the student is trying to answer: ${questions[num + 1]}`
    }));
    console.log("Updated system prompt:", systemPrompt.content); // Log the updated system prompt

    // Optional: Force a rerender by changing a key or similar
    // setKey(prevKey => prevKey + 1); // If you have a key state
  }

  console.log("selected question for display: " + selectedQuestion )
  return (
    <div className="font-orienta h-screen bg-[#F8F7F4] px-12 pt-10 pb-12 flex gap-5">
      ...
      <Display selectedNum = {selectedNum} selectedQuestion={selectedQuestion}/>
    </div>
  );
}